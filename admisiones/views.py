...
def menuAcceso(request):
    ...
    #miConexion = MySQLdb.connect(host='localhost', user='sa', passwd='75AAbb??', db='vulnerable')
    miConexion = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
    #miConexion = pyodbc.connect('DRIVER={SQL Server};SERVER=CMKSISTEPC07\MEDICAL;DATABASE=vulnerable ; Trusted_Connection=yes ; UID=sa;pwd=75AAbb??')
    cur = miConexion.cursor()
    comando = "SELECT id ,nombre, 'modulosid_id', 'sedesClinica_id' FROM seguridad_perfilesclinica"
    cur.execute(comando)
    print(comando)
    ...
    for id, nombre , modulosid_id, sedesClinica_id in cur.fetchall():
        perfiles.append({'id': id, 'nombre': nombre,'modulosid_id':modulosid_id, 'sedesClinica_id':sedesClinica_id})
...
def validaAcceso(request):
    ...
    context['NombreSede'] =  'CLINICA DE PRUBAS'
    ...
    miConexion0 = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
    ...
    if planta == []:
        ...
        miConexion1 = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
        ...
        if plantaContrasena == []:
            ...
            miConexion2 = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
            ...
            if perfil == []:
                ...
                miConexionx = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
              ...
              #  detalle = "SELECT  tp.nombre tipoDoc,  u.documento documento, u.nombre  nombre , i.consec consec , fechaIngreso , fechaSalida, ser.nombre servicioNombreIng, dep.nombre camaNombreIng , dxIngreso_id FROM admisiones_ingresos i, usuarios_usuarios u, sitios_dependencias dep , clinico_servicios ser ,usuarios_tiposDocumento tp  WHERE i.sedesClinica_id = dep.sedesClinica_id AND i.serviciosActual_id = dep.servicios_id AND i.serviciosActual_id = ser.id  AND i.dependenciasActual_id = dep.id AND i.sedesClinica_id= '" +  str(Sede) + "' AND i.salidaDefinitiva = 'N' and tp.id = u.tipoDoc_id"
                ...
                miConexiont = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
            ...
            #return render(request, "Reportes/PantallaPrincipal.html", context)


    return render(request, "Reportes/PantallaPrincipal.html", context)


def escogeAcceso(request, Sede, Username, Perfil, Documento, NombreSede, escogeModulo ):
    ...
    #sede = request.POST["seleccion2"]
    #Sede = sede
    #permisoGral = request.POST["permisosGrales.id"]
    ...
    miConexiont = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres",
                                   password="BD_m3d1c4l")
    curt = miConexiont.cursor()
    ...
    # miConexionx = MySQLdb.connect(host='CMKSISTEPC07', user='sa', passwd='75AAbb??', db='vulnerable')
    miConexionx = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres",
                                   password="BD_m3d1c4l")
    curx = miConexionx.cursor()
...
def buscarHabitaciones(request):
    ...
    if Exc == 'N':
      ...
      comando = "SELECT dep.id id ,dep.numero nombre  FROM sitios_serviciosSedes sed, clinico_servicios ser  , sitios_subserviciossedes sub , sitios_dependencias dep  Where sed.sedesClinica_id ='" + str(
       ...
       comando = 'SELECT dep.id id ,dep.numero nombre FROM sitios_serviciosSedes sed, clinico_servicios ser  , sitios_subserviciossedes sub , sitios_dependencias dep , sitios_dependenciasActual act Where sed."sedesClinica_id" =' + "'" + str(sede) + "'" +' AND sed."servicios_id" = ser.id and  sed."sedesClinica_id" = sub."sedesClinica_id" and sed."servicios_id" = sub."servicios_id" and  dep."sedesClinica_id"=sed."sedesClinica_id" and dep."serviciosSedes_id" = sub."servicios_id" and dep."subServiciosSedes_id" = sub.id and dep."subServiciosSedes_id" = ' + "'" + str(SubServ) + "'" + ' and act."dependencias_id" = dep.id and act.disponibilidad = ' + "'" + str('L') + "'" + ' UNION SELECT dep.id id ,dep.numero nombre FROM sitios_serviciosSedes sed, clinico_servicios ser , sitios_subserviciossedes sub , sitios_dependencias dep Where sed."sedesClinica_id" =' + "'"' + str(sede) + "'" +' AND sed."servicios_id" = ser.id and sed."sedesClinica_id" = sub."sedesClinica_id" and sed."servicios_id" = sub."servicios_id" and dep."sedesClinica_id" = sed."sedesClinica_id" and dep."serviciosSedes_id" = sub."servicios_id" and dep."subServiciosSedes_id" =sub.id and dep."subServiciosSedes_id" = ' + "'" + str(SubServ) + "'" + ' and not EXISTS  (select act.id from sitios_dependenciasactual act where act."dependencias_id" = dep.id)'
...
def guardarUsuariosModal(request):
    ...
    miConexion11 = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
    ...
    if Usuarios == []:
         ...
         miConexion3 = psycopg2.connect(host="192.168.0.51", database="vulner", port="5432", user="postgres", password="BD_m3d1c4l")
        ...
        miConexion3 = pyodbc.connect(
            'DRIVER={SQL Server};SERVER=CMKSISTEPC07\MEDICAL;DATABASE=vulnerable ;Trusted_Connection=yes ; UID=sa;pwd=75AAbb??')
        cur3 = miConexion3.cursor()
